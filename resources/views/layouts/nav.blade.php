...
<nav class="bg-[#222] border-b z-50 border-gray-600 fixed top-0 left-0 w-full">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-6">
        <!-- the logo -->
        <a href="{{route('books')}}" class="flex items-center grow space-x-3 rtl:space-x-reverse">
        ...
        <?php if($_SERVER['REQUEST_URI'] == '/wiki/home' or $_SERVER['REQUEST_URI'] == '/wiki/') : ?>
            <div class="search-wrapper grow max-w-md invisible">
                ...
                <div class="relative">
                    ...
                    <input type="search" id="default-search" class="search-input block w-full p-3.5 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Search..." required>
                    <button type="submit" class="search-btn text-white absolute end-2.5 bottom-1.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2">Search</button>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div class="hidden grow w-full md:block md:w-auto" id="navbar-default">
            <ul class="font-medium text-center flex flex-col justify-end px-4 py-10 md:p-0 mt-4 border border-gray-100 rounded-lg md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0">
                <li>
                    <a href="/" class="<?= ($_SERVER['REQUEST_URI'] == '/wiki/home' or $_SERVER['REQUEST_URI'] == '/wiki/')? 'text-blue-500' : 'text-white';?> block py-2 px-3 bg-blue-700 rounded md:bg-transparent md:p-0 " >Home</a>
                </li>
                <li>
                    <a href="{{route('book.create')}}" class="<?= ($_SERVER['REQUEST_URI'] === '/wiki/create') ? 'text-blue-500' : 'text-white' ; ?> block mb-8 md:mb-0 py-2 px-3 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:p-0">Create</a>
                </li>
                @if (!Auth::check())
                    <li>
                        <a href="{{route('login')}}" class="<?= ($_SERVER['REQUEST_URI'] === '/wiki/signin') ? 'text-blue-500' : 'text-white' ; ?> block py-2 px-3 rounded md:border-0 md:p-0">Sign In</a>
                    </li>
                    <li>
